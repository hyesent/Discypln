// ============================================================
//  STATS MODAL — Full Stats Dashboard (Self-Contained)
...
// ============================================================
//  STATS MODAL COMPONENT (Self-Contained)
// ============================================================

export default function StatsModal({ onClose, user, supabase, showToast }) {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      // Fetch all data
      const [tasksRes, journalRes, pomodoroRes] = await Promise.all([
        supabase.from('tasks').select('*').eq('user_id', user.id),
        supabase.from('journal').select('*').eq('user_id', user.id),
        supabase.from('pomodoro_sessions').select('*').eq('date', today).eq('user_id', user.id)
      ])

      const tasks = tasksRes.data || []
      const journalEntries = journalRes.data || []
      const totalMinutes = pomodoroRes.data?.[0]?.total_minutes || 0
      ...
      // ===== DAILY AVERAGE =====
      const dailyAverage = parseFloat((doneTasks / 30).toFixed(1))
      ...
      const avgFocusPerDay = monthFocus > 0 ? Math.round(monthFocus / 30) : 0
      ...
      // ===== DISCIPLINE SCORE =====
      const streakWeight = Math.min(currentStreak / 100, 1) * 35
      ...
      // ===== TASK VELOCITY =====
      const taskVelocity = totalMinutes > 0 ? Number((doneTasks / (totalMinutes / 60)).toFixed(1)) : 0

      // ===== WEEKLY GOAL PROGRESS =====
      const weeklyGoalProgress = weeklyBreakdown.reduce((acc, d) => acc + d.percent, 0) / 7

      // ===== AVERAGE TASK TIME =====
      const completedTasksWithTime = tasks.filter(t => t.done && t.estimated_minutes)
      const averageTaskTime = completedTasksWithTime.length > 0
        ? Math.round(completedTasksWithTime.reduce((sum, t) => sum + t.estimated_minutes, 0) / completedTasksWithTime.length)
        : 0
    ...
    return (
      <div className="stats-modal-overlay" onClick={onClose}>
        <div className="stats-modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="stats-modal-content" style={{ textAlign: 'center', padding: '40px' }}>
            <div className="skeleton skeleton-title" style={{ width: '60%', margin: '0 auto 16px' }} />
            <div className="skeleton skeleton-text" style={{ width: '80%', margin: '0 auto 8px' }} />
