...
export default function App() {
  ...
  // ===== Navigation =====
  const [currentView, setCurrentView] = useState('dashboard') // 'dashboard' | 'notes' | 'tasks' | 'journal'
  const [activeTab, setActiveTab] = useState('notes') // for capsule nav inside tabs
  ...
  // ============================================================
  //  LOCAL STORAGE CACHE (7-day retention)
  // ============================================================
  ...
  const addToTrash = (type, item) => {
    ...
    setTrash(prev => ({
      ...prev,
      [type]: [...prev[type], trashItem]
    }))
    // Save trash to localStorage
    try {
      localStorage.setItem('discypln_trash', JSON.stringify(trash))
    } catch (e) {}
  }
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('discypln_trash')
      if (saved) {
        ...
        Object.keys(parsed).forEach(key => {
          filtered[key] = parsed[key].filter(item => 
  ...
  // Save trash to localStorage when it changes
  useEffect(() => {
    try {
      localStorage.setItem('discypln_trash', JSON.stringify(trash))
    } catch (e) {}
  ...
  const calculateAllStats = useCallback(() => {
    ...
    const completionRate = totalTasksEver > 0 ? Math.round((doneTasksCount / totalTasksEver) * 100) : 0
    
    const tasksWithTime = tasks.filter(t => t.estimated_minutes)
    const avgTaskTime = tasksWithTime.length > 0 
      ? Math.round(tasksWithTime.reduce((sum, t) => sum + t.estimated_minutes, 0) / tasksWithTime.length) 
      : 0
    
    const hourCounts = {}
    tasks.filter(t => t.done && t.time).forEach(t => {
    ...
    const weekTasks = tasks.filter(t => new Date(t.updated_at || t.due_date) >= weekStart)
    const weekDone = weekTasks.filter(t => t.done).length
    const weeklyGoalProgress = weekTasks.length > 0 ? Math.round((weekDone / weekTasks.length) * 100) : 0
  ...
  const pauseSession = () => {
    ...
    showToast('⏸️ Session paused', 'info')
  }

  const resumeSession = () => {
    setSessionRunning(true)
    setSessionPaused(false)
    showToast('▶️ Session resumed', 'info')
  }
  ...
  const stopSession = () => {
    ...
    showToast('⏹️ Session stopped', 'info')
  }
  ...
  const exportStatsReport = () => {
    ...
    doc.setFontSize(16)
    doc.setTextColor(0, 0, 0)
    doc.text('⏱️ Focus Analysis', 20, y)
    y += 10
  ...
  const fetchAllData = async () => {
    setLoadingStates({ notes: true, tasks: true, journal: true, stats: true })
    ...
    setLoadingStates({ notes: false, tasks: false, journal: false, stats: false })
  }
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval)
  }, [sessionRunning, sessionPomodoroTime])
  ...
  async function deleteNote(id) {
    ...
    if (error) {
      ...
      showToast('Note moved to trash', 'success', () => restoreNote(id))
      await fetchNotes()
      setViewMode('home')
    }
  }
  ...
  function restoreNote(id) {
    const item = restoreFromTrash('notes', id)
    if (item) {
  ...
  async function deleteTask(id) {
    ...
    if (!error) {
      ...
      showToast('Task moved to trash', 'success', () => restoreTask(id))
      fetchTasks()
    }
  }

  function restoreTask(id) {
    const item = restoreFromTrash('tasks', id)
    if (item) {
  ...
  async function deleteJournalEntry(id) {
    ...
    if (!error) {
      ...
      showToast('Entry moved to trash', 'success', () => restoreJournalEntry(id))
      fetchJournal()
    }
  }

  function restoreJournalEntry(id) {
    const item = restoreFromTrash('journal', id)
    if (item) {
      setJournalEntries(prev => [item, ...prev])
      showToast('Journal entry restored!', 'success')
    }
  }
        ...
        <div className="container fade-in" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
            ...
            <div 
              ...
              onClick={() => { setCurrentView('notes'); setActiveTab('notes'); }}
            ...
            <div 
              className="card card-glow" 
              style={{ cursor: 'pointer', padding: '24px', transition: 'all 0.2s ease' }}
              onClick={() => { setCurrentView('tasks'); setActiveTab('tasks'); }}
              onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.borderColor = 'var(--glass-border-hover)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'var(--glass-border)'; }}
            >
              <div style={{ fontSize: '32px', marginBottom: '8px' }}>✅</div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span className="tiny-label">{tasks.filter(t => !t.done).length} pending • {tasks.filter(t => t.done).length} done</span>
                <span style={{ color: 'var(--brand-blue)', fontSize: '14px', fontWeight: 500 }}>Open →</span>
              </div>
            </div>
            ...
            <div 
              className="card card-glow" 
              style={{ cursor: 'pointer', padding: '24px', transition: 'all 0.2s ease' }}
              onClick={() => { setCurrentView('journal'); setActiveTab('journal'); }}
              onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.borderColor = 'var(--glass-border-hover)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'var(--glass-border)'; }}
            >
              <div style={{ fontSize: '32px', marginBottom: '8px' }}>📖</div>
            ...
            <div className="capsule-nav-inner">
              {['notes', 'tasks', 'journal'].map((tab) => (
                <button
                  ...
                  onClick={() => {
                    setActiveTab(tab)
                    if (tab === 'notes') setCurrentView('notes')
                    else if (tab === 'tasks') setCurrentView('tasks')
                    else if (tab === 'journal') setCurrentView('journal')
                  }}
                >
    ...
    return (
      <>
        ...
        <div className="container fade-in" style={{ position: 'relative', zIndex: 1 }}>
          ...
          {/* ===== TASKS CONTENT ===== */}
          <div>
            <div className="section-title">Tasks Dashboard</div>
            ...
            <div className="card" style={{ marginBottom: '16px', padding: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: '12px' }}>
                <div className="stat-item">
                  <div className="stat-value" style={{ color: '#F59E0B' }}>{tasks.filter(t => t.done).length}</div>
                  <div className="stat-label">✅ Done</div>
                </div>
                <div className="stat-item">
                  <div className="stat-value" style={{ color: 'var(--brand-blue)' }}>{tasks.filter(t => !t.done).length}</div>
                  <div className="stat-label">⏳ Pending</div>
                </div>
                <div className="stat-item">
                  <div className="stat-value" style={{ color: '#22C55E' }}>{formatMinutes(totalMinutes)}</div>
                  <div className="stat-label">⏱️ Focus Time</div>
                </div>
                <div className="stat-item">
                  <div className="stat-value" style={{ color: '#A855F7' }}>{pomodoroSessions}</div>
                  <div className="stat-label">🎯 Sessions</div>
                </div>
                <div className="stat-item">
                  <div className="stat-value" style={{ color: '#F97316' }}>{statsData.completionRate}%</div>
                  <div className="stat-label">📊 Completion</div>
                </div>
                <div className="stat-item">
                  <div className="stat-value" style={{ color: 'var(--brand-purple)' }}>{statsData.habitCompletion}%</div>
                  <div className="stat-label">🎯 Habits</div>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              ...
              <div className="card">
                ...
                <div style={{ marginTop: '6px', fontSize: '11px', color: 'var(--text-tertiary)' }}>
                  {tasks.filter(t => t.done).length} tasks done
                </div>
              </div>
            </div>
            ...
            {/* ===== TASK LIST ===== */}
            <div className="section-title" style={{ marginTop: '16px' }}>Your Tasks</div>
                        ...
                        >
                          ...
                          {isActiveSession && (
                            <div style={{
                              ...
                              <button 
                                onClick={completeTaskFromSession} 
      ...
      <div className="stats-modal-overlay" onClick={() => setShowStatsModal(false)}>
        <div className="stats-modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="stats-modal-content">
            ...
            {/* 4. Focus Analysis */}
            <div className="stats-section">
              <div className="stats-section-title">⏱️ Focus Analysis</div>
              <div className="stats-section-divider" />
              <div style={{ 
                display: 'grid', 
  ...
  if (showTrashModal) {
    ...
    return (
      <div className="stats-modal-overlay" onClick={() => setShowTrashModal(false)}>
        <div className="stats-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '700px' }}>
          <div className="stats-modal-header">
            <div className="stats-modal-title">🗑️ Trash</div>
            <button className="stats-modal-close" onClick={() => setShowTrashModal(false)}>
              <IconX size={20} />
            </button>
          </div>
          ...
          <div className="stats-modal-content">
            {totalTrash === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize: '14px', marginTop: '4px', color: 'var(--text-tertiary)' }}>
                  Deleted items appear here for 7 days.
                </p>
              </div>
            ) : (
              <>
                ...
                {trash.notes.length > 0 && (
                  <div className="stats-section">
                    <div className="stats-section-title">📝 Notes ({trash.notes.length})</div>
                    <div className="stats-section-divider" />
                    {trash.notes.map((item) => (
                      <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--glass-border)' }}>
                        <div>
                          <div style={{ fontWeight: 600, fontSize: '14px' }}>{item.title}</div>
                          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Deleted: {formatDate(item.deletedAt)}</div>
                        </div>
                        <button 
                    ...
                    <button 
                      onClick={() => { emptyTrash('notes'); showToast('Notes trash emptied', 'success') }}
                      className="btn btn-sm btn-danger"
                ...
                {trash.tasks.length > 0 && (
                  <div className="stats-section">
                    <div className="stats-section-title">✅ Tasks ({trash.tasks.length})</div>
                    <div className="stats-section-divider" />
                    ...
                    <button 
                      onClick={() => { emptyTrash('tasks'); showToast('Tasks trash emptied', 'success') }}
                      className="btn btn-sm btn-danger"
                      style={{ marginTop: '8px', height: '28px', padding: '0 12px', fontSize: '11px' }}
                    >
                      Empty Tasks Trash
                ...
                {trash.journal.length > 0 && (
                  <div className="stats-section">
                    <div className="stats-section-title">📖 Journal ({trash.journal.length})</div>
                    <div className="stats-section-divider" />
                    {trash.journal.map((item) => (
                      <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--glass-border)' }}>
                        <div>
                          <div style={{ fontWeight: 600, fontSize: '14px' }}>{item.content?.slice(0, 50)}...</div>
                          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Deleted: {formatDate(item.deletedAt)}</div>
                        </div>
                        <button 
                          onClick={() => restoreJournalEntry(item.id)}
                          className="btn btn-sm btn-primary"
                          style={{ height: '28px', padding: '0 12px', fontSize: '11px' }}
                        >
                          Restore
                    ...
                    <button 
                      onClick={() => { emptyTrash('journal'); showToast('Journal trash emptied', 'success') }}
                      className="btn btn-sm btn-danger"
                      style={{ marginTop: '8px', height: '28px', padding: '0 12px', fontSize: '11px' }}
                    >
                      Empty Journal Trash
